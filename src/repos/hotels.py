...
class HotelsRepository(BaseRepository):
    ...
    async def get_filtered_by_time(self, date_from: date, date_to: date, location, title, limit=None, offset=None):
        rooms_ids_to_get = await rooms_ids_for_booking(date_from, date_to)
        hotels_ids = select(RoomsOrm.hotel_id).select_from(RoomsOrm).filter(RoomsOrm.id.in_(rooms_ids_to_get))
