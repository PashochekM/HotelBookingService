...
class RoomsFacilitiesRepository(BaseRepository):
    ...
    async def set_room_facilities(self, room_id: int, f_ids: list[int]):
        get_current_f_ids = select(self.model.facility_id).filter_by(room_id=room_id)
        ...
        if ids_to_delete:
            delete_m2m_f_stmt = delete(self.model).filter(self.model.room_id == room_id, self.model.facility_id.in_(ids_to_delete))
